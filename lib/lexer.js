...
const checkFragments = function(lexed) {
  const fragmentNames = lexed
    .filter((item) => item.definition === dict.FRAGMENT_NAME)
    .map((item) => item.value);

  const fragmentDeclarations = lexed
    .filter((item) => item.definition === dict.FRAGMENT_DECLARATION)
    .map((item) => item.value);

  fragmentNames.forEach((name) => {
    if (!fragmentDeclarations.includes(name)) {
      const declarations = `[${fragmentDeclarations.join(", ")}]`;
      let error = `Syntax error: Fragment '${name}' has no definition.  `;
      error += `Available fragment definitions: ${declarations}`;
      throw Error(error);
    }
  });
};
